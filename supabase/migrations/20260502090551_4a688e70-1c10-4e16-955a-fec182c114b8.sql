...
-- 6) Marca come demo TUTTI i contesti dell'account admin (admin@example.com).
--    Lookup via auth.users per ottenere lo UUID corrispondente.
UPDATE public.study_contexts sc
SET is_demo = true
WHERE sc.user_id IN (
  SELECT u.id::text FROM auth.users u WHERE u.email = 'admin@example.com'
)
OR sc.user_id = 'admin@example.com'; -- fallback per eventuale legacy user_id
...
AS $$
  SELECT EXISTS (
    ...
    WHERE u.id::text = _user_id AND u.email = 'admin@example.com'
  );
$$;