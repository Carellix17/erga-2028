...
export function BottomNav({ activeTab, onTabChange }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 px-4 pointer-events-none md:static md:z-auto md:px-0 md:pointer-events-auto md:w-64 md:h-screen md:sticky md:top-0 md:self-start md:shrink-0">
      ...
      <div
        className="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-background via-background/85 to-transparent md:hidden"
        aria-hidden
      />
      <div className="max-w-lg mx-auto pointer-events-auto mb-[max(env(safe-area-inset-bottom,0px),1rem)] flex items-center gap-2.5 md:max-w-none md:mx-0 md:mb-0 md:h-full md:items-stretch">
        ...
        <div className={cn(pillMaterial, "rounded-pill shadow-level-2 flex-1 min-w-0 md:rounded-none md:border-0 md:border-r md:h-full md:flex md:flex-col")}>
          {/* Telefono: pillola con SOTTO-PILLOLA FLUIDA (layoutId) */}
          <div className="relative grid grid-cols-3 items-center justify-items-center h-[4.5rem] px-2 rounded-pill md:hidden">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={cn("w-1.5 h-1.5 rounded-pill", isActive ? "bg-primary-foreground" : "bg-transparent")} />
                  </button>
                );
              })}
            </div>
